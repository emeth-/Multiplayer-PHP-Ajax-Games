	...
	if($username)
	{
		...
		else
		{
			...
			if($ga['time_left'] > 40)
			{
				if($ga['notifyturn']==0)
				{
					$timetxt .="<form method='post' name='n1'><input type=hidden name='notify' value=1><input type='submit' border=0 name='nbutton' id='nbutton' value='Notify opponent via Facebook it&lsquo;s their turn!' onclick=\"postFormAjax('cf_play.php', 'n1');return false;\" /></form>";
				}
...
if($gi['red']==$ir['userid'])
{
	$mymarkis = " You are red.";
}
else
{
	$mymarkis = " You are blue.";
}
...
{
	$turntxt = "<h3><font color=green>Your Turn!{$mymarkis}</font></h3>";
}
...
$rat1 = $db->query("SELECT rating FROM cf_ranks WHERE userid={$ga['p1']}");
...
$rat2 = $db->query("SELECT rating FROM cf_ranks WHERE userid={$ga['p2']}");
