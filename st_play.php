...
{
	if($ir['userid']==$gi["$firstmovemarkfield"]){$whichc = "Red ";}else{$whichc = "Blue ";}
	$topturntxt = "<h3><font color=green>Your Turn! You are {$whichc}</font></h3>";
}
else if($gi['winner']==0 && $gamerunning>0 && $gi['mode'] == 1) 
{
	$topturntxt = "<h3><font color=red>Opponent's Turn.</font></h3>";
} 
else if($gi['winner']==0 && $gamerunning>0 && $gi['mode'] == 0) 
{
	if($ir['userid'] == $gi['red'] && $gi['p1ready'] == 1)
	{
		$topturntxt = "<h3><font color=red>Waiting for opponent to place their pieces!</font></h3>";
	}
	else if($ir['userid'] != $gi['red'] && $gi['p2ready'] == 1)
	{
		$topturntxt = "<h3><font color=red>Waiting for opponent to place their pieces!</font></h3>";
	}
	else
	{
		$topturntxt = "<h3><font color=green>Place your pieces! (Click a piece to select it)</font></h3>";	
	}
}
$rat1 = $db->query("SELECT rating FROM st_ranks WHERE userid={$ga['p1']}");
...
$rat2 = $db->query("SELECT rating FROM st_ranks WHERE userid={$ga['p2']}");
