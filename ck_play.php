	...
	if($username)
	{
		...
		else
		{
			...
			if($ga['time_left'] > 40)
			{
				if($ga['notifyturn']==0)
				{
					$timetxt .="<form method='post' name='n1'><input type=hidden name='notify' value=1><input type='submit' border=0 name='nbutton' id='nbutton' value='Notify opponent via Facebook it&lsquo;s their turn!' onclick=\"postFormAjax('ck_play.php', 'n1');return false;\" /></form>";
				}
...
{
	if($ir['userid']==$gi["$firstmovemarkfield"]){$whichc = "Red ";$arrow="barrow";}else{$arrow="warrow";$whichc = "White ";}
...
$rat1 = $db->query("SELECT rating FROM ck_ranks WHERE userid={$ga['p1']}");
...
$rat2 = $db->query("SELECT rating FROM ck_ranks WHERE userid={$ga['p2']}");
