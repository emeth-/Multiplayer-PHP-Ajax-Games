	...
	if($username)
	{
		...
		else
		{
			...
			if($ga['time_left'] > 40)
			{
				if($ga['notifyturn']==0)
				{
					$timetxt .="<form method='post' name='n1'><input type=hidden name='notify' value=1><input type='submit' border=0 name='nbutton' id='nbutton' value='Notify opponent via Facebook it&lsquo;s their turn!' onclick=\"postFormAjax('bg_play.php', 'n1');return false;\" /></form>";
				}
...
{
	if($ir['userid']==$gi["$firstmovemarkfield"]){$whichc = "Black ";$arrow="barrow";}else{$arrow="warrow";$whichc = "White ";}
	$topturntxt = "<h3><font color=green>Your Turn! You are {$whichc}<img src='images/bg/{$arrow}.jpg'></font></h3><font size=1>Click one of the dice to select it, then click a piece to move it.</font><br /><br />";
}
...
$rat1 = $db->query("SELECT rating FROM bg_ranks WHERE userid={$ga['p1']}");
...
$rat2 = $db->query("SELECT rating FROM bg_ranks WHERE userid={$ga['p2']}");
...
if($sdie)
{
	...
	select_dice($sdie, $userid, $roomid);
}
