...
if($ga['play_time']>0 && $gi['winner']==0) //only if the game has started / first move made
{
	...
	if($username)
	{
		...
		if($timeleft <= 0)
		{
			if($ga['p1']==$ga['turn']){$gain = $ga['p2'];}
			if($ga['p2']==$ga['turn']){$gain = $ga['p1'];}
			if($gain == $gi['bottom']){$db->query("UPDATE {$gpre}game SET b7=b7+3 WHERE {$gpre}room=$roomid");}
			if($gain != $gi['bottom']){$db->query("UPDATE {$gpre}game SET b0=b0+3 WHERE {$gpre}room=$roomid");}
			
			//lost their turn
			toggle_turn($roomid);
			$txt = "$username ran out of time and lost his/her move. Opponent gained 3 points.";
		...
		else
		{
			...
			if($ga['time_left'] > 40)
			{
				if($ga['notifyturn']==0)
				{
					$timetxt .="<form method='post' name='n1'><input type=hidden name='notify' value=1><input type='submit' border=0 name='nbutton' id='nbutton' value='Notify opponent via Facebook it&lsquo;s their turn!' onclick=\"postFormAjax('man_play.php', 'n1');return false;\" /></form>";
				}
...
{
	if($gi['bottom']==$ir['userid'])
	{
		$botturntxt = "<h3><font color=green>Your Turn!</font></h3>";
	}
	else
	{
		$topturntxt = "<h3><font color=green>Your Turn!</font></h3>";
	}
}
else if($gi['winner']==0 && $gamerunning>0) 
{
	if($gi['bottom']!=$ir['userid'])
	{
		$botturntxt = "<h3><font color=red>Opponent's Turn.</font></h3>";
	}
	else
	{
		$topturntxt = "<h3><font color=red>Opponent's Turn.</font></h3>";
	}
}

$rat1 = $db->query("SELECT rating FROM man_ranks WHERE userid={$ga['p1']}");
...
$rat2 = $db->query("SELECT rating FROM man_ranks WHERE userid={$ga['p2']}");
